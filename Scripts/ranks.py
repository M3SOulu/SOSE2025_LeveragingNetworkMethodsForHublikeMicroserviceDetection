...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "spearman_correlation_heatmap.pdf"))
plt.close()
...
plt.ylabel("Distance")
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "centrality_dendrogram.pdf"))
plt.close()
...
plt.yticks(rotation=0)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "nemenyi_posthoc_heatmap.pdf"))
plt.close()